...
type
  TDMAcademico = class(TDataModule)
    ...
    function existe_disciplina(COD_ALUNO,COD_DISCIPLINA:Integer):boolean;
  private
...
function TDMAcademico.existe_disciplina(COD_ALUNO,COD_DISCIPLINA:Integer):boolean;
begin
  FDQry_Exist_Disciplina.close;
  FDQry_Exist_Disciplina.ParamByName('COD_ALUNO').AsInteger := COD_ALUNO;
  FDQry_Exist_Disciplina.ParamByName('COD_DISCIPLINA').AsInteger := COD_DISCIPLINA;
...
begin
  ...
  if FDQry_aluno_DisciplinaCOD_DISCIPLINA.OldValue <> FDQry_aluno_DisciplinaCOD_DISCIPLINA.Value then
  begin
    ...
    begin
      MessageDlg('Aluno já matricukado na Disciplina', mtWarning, [mbOK], 0);
      abort;
    end;
  end;
